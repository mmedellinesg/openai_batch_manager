...
class LogFileManager:
    def __init__(self, path):
        self.path = path
        self.columns = ['id', 'submission_datetime', 'purpose','infile','infile_id','status','status_datetime']
        self._ensure_exists()
    ...
    def log_submission(self, batch_id, infile, infile_id, purpose, status='created'):
        df = pd.read_csv(self.path)
        row = pd.DataFrame([[batch_id, datetime.now().isoformat(), purpose, infile, infile_id, status, datetime.now().isoformat()]], columns=self.columns)
